-- Complete fix voor user_sessions tabel schema
-- Lost alle ontbrekende kolommen op
...
BEGIN
    ...
    -- Update kolommen die mogelijk een andere naam hebben
    -- start_time -> started_at mapping
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
    ...
    -- Als we zowel start_time als started_at hebben, gebruik started_at en drop start_time
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_sessions' AND column_name = 'start_time'
    ) AND EXISTS (
        ...
        ALTER TABLE user_sessions DROP COLUMN start_time;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE 'Dropped existing session foreign key constraint';
    END IF;
...
-- Test of alles werkt
SELECT 'user_sessions schema volledig gefixed!' as status; 