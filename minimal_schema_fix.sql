-- ====================================================================
-- Minimal Enterprise Database Schema Fix
-- Task 65.1: Add only essential missing columns to avoid syntax errors
-- ====================================================================

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- First ensure all missing columns exist in content_posts
DO $$
BEGIN
    -- Add status column
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_posts' AND column_name = 'author_id') THEN
        ALTER TABLE content_posts ADD COLUMN author_id UUID;
    ...
    -- Add target_platforms column as JSONB
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'social_accounts' AND column_name = 'user_id') THEN
        ALTER TABLE social_accounts ADD COLUMN user_id UUID;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_calendar' AND column_name = 'assigned_to') THEN
        ALTER TABLE content_calendar ADD COLUMN assigned_to UUID;
...
-- Create only essential B-tree indexes (no GIN indexes to avoid issues)
CREATE INDEX IF NOT EXISTS idx_content_posts_status_btree ON content_posts(status);
CREATE INDEX IF NOT EXISTS idx_content_posts_campaign_id_btree ON content_posts(campaign_id);
CREATE INDEX IF NOT EXISTS idx_content_posts_author_id_btree ON content_posts(author_id);
CREATE INDEX IF NOT EXISTS idx_content_posts_published_at_btree ON content_posts(published_at);
CREATE INDEX IF NOT EXISTS idx_content_posts_organization_id_btree ON content_posts(organization_id);

CREATE INDEX IF NOT EXISTS idx_social_accounts_status_btree ON social_accounts(status);
CREATE INDEX IF NOT EXISTS idx_social_accounts_platform_btree ON social_accounts(platform);
CREATE INDEX IF NOT EXISTS idx_social_accounts_user_id_btree ON social_accounts(user_id);
CREATE INDEX IF NOT EXISTS idx_social_accounts_organization_id_btree ON social_accounts(organization_id);

CREATE INDEX IF NOT EXISTS idx_content_calendar_status_btree ON content_calendar(status);
CREATE INDEX IF NOT EXISTS idx_content_calendar_date_btree ON content_calendar(calendar_date);
CREATE INDEX IF NOT EXISTS idx_content_calendar_campaign_id_btree ON content_calendar(campaign_id);
CREATE INDEX IF NOT EXISTS idx_content_calendar_assigned_to_btree ON content_calendar(assigned_to);
CREATE INDEX IF NOT EXISTS idx_content_calendar_organization_id_btree ON content_calendar(organization_id);
